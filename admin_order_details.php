...
From user_reg Inner Join
  orders On user_reg.UID = orders.customer_id order by orders.id asc";
...
$sql="Select orders.id, user_reg.UNAME, orders.total_price, orders.created,user_reg.UID ,
  orders.status
From user_reg Inner Join
  orders On user_reg.UID = orders.customer_id order by orders.id asc $limit";
					$result=$con->query($sql);
 ...
 $result=$con->query($sql);
					...
					<thead class="text-primary">
								...
								else
								{
									...
									$list.="<a href='admin_Update_order.php?id=".$row["id"]."' class='btn btn-danger btn-xs'><i class='fa fa-eye'></i> Pending</a>";
